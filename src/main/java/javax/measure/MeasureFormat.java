...
public abstract class MeasureFormat extends Format {
	...
	protected Appendable formatCompound(double value, CompoundUnit<?> unit,
		...
		if (high instanceof CompoundUnit)
			...
			dest.append(DEFAULT.numberFormat.format(highValue));
			DEFAULT.unitFormat.format(high, dest);
		}
		dest.append(DEFAULT.numberFormat.format(lowValue));
		return DEFAULT.unitFormat.format(low, dest);
	}
	...
	private static final class NumberSpaceUnit extends MeasureFormat {
		 ...
		 * 
		 */
		private static final long serialVersionUID = -4737128421865677535L;
		...
		public Measure<?> parse(CharSequence csq, ParsePosition cursor)
			...
			Unit unit = unitFormat.parse(csq, cursor);
