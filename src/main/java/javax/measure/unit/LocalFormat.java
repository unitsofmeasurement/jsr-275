 ...
 * @version 1.3 ($Revision: 134 $), $Date: 2010-02-17 21:55:53 +0100 (Mi, 17 Feb 2010) $
 */
class LocalFormat extends UnitFormat {
    ...
    private static final String BUNDLE_NAME = LocalFormat.class.getPackage().getName() + ".messages";
    ...
    private int formatInternal(Unit<?> unit, Appendable buffer) throws IOException {
        ...
        if (symbol != null) {
        ...
        } else if ((unit instanceof TransformedUnit<?>) || unit.equals(MetricSystem.KILOGRAM)) {
            ...
            if (unit.equals(MetricSystem.KILOGRAM)) {
                ...
                converter = ParsePrefix.KILO.getConverter();
                unitPrecedence = formatInternal(MetricSystem.GRAM, temp);
                printSeparator = true;
            } else {
                ...
                if (parentUnits.equals(MetricSystem.KILOGRAM)) {
                    ...
                    converter = converter.concatenate(ParsePrefix.KILO.getConverter());
                }
            ...
            return NOOP_PRECEDENCE;
        } else { // Unknown unit, we use its String representation (UCUM or overriden).
